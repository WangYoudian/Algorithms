...
if __name__ == '__main__':
    ...
    N = 1000
    # a,b
    m_str = ''.join(list(map(lambda x:random.choice(alphabet) , [i for i in range(m_size)])))
    p_str = ''.join(list(map(lambda x:random.choice(alphabet) , [i for i in range(p_size)])))
    start = time.clock()
    for i in range(N):
    	kmp(m_str, p_str)
    end = time.clock()
    ...
    print('[Built-in Functions] time:', end-start)

    print(m_str.find(p_str))
    print(m_str)
    print(' '*(m_str.find(p_str)-1),p_str)
